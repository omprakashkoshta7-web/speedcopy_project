...
const BrochuresListPage: React.FC = () => {
  ...
  const fetchBrochures = async () => {
    try {
      setLoading(true);
      const response = await productService.getBusinessProducts({ category: 'brochures', limit: 12 });
      setBrochures(response.data.products || []);
    } catch (err) {
    ...
    <div style={{ backgroundColor: '#f0f0f0', minHeight: '100vh' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Brochures</h1>
          <p className="text-gray-600">Elegant multi-page layouts for detailed presentations</p>
        </div>

        {/* Products Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
          {brochures.map((product, index) => (
            <div
            ...
            >
              ...
              <div
                ...
                style={{ height: '200px', backgroundColor: product.bg || '#d4c5f9' }}
              >
                <div className="flex items-center justify-center w-full h-full px-8">
                  <div className="bg-white rounded-xl shadow-md w-full max-w-[200px] p-4 flex flex-col gap-2">
                    <div className="h-3 rounded-full bg-gray-200 w-full" />
                    <div className="h-1.5 rounded-full bg-gray-100 w-5/6" />
                    <div className="h-1.5 rounded-full bg-gray-100 w-4/6" />
                    <div className="h-1 rounded-full bg-gray-100 w-full mt-2" />
                    <div className="h-1 rounded-full bg-gray-100 w-3/4" />
                    <div className="h-1 rounded-full bg-gray-100 w-5/6" />
                    <div className="h-1 rounded-full bg-gray-100 w-2/3 mt-1" />
                  </div>
                </div>
                {product.badge && (
...
export default BrochuresListPage;