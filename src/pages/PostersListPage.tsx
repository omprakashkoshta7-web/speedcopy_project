...
const PostersListPage: React.FC = () => {
  ...
  const fetchPosters = async () => {
    try {
      setLoading(true);
      const response = await productService.getBusinessProducts({ category: 'posters', limit: 12 });
      setPosters(response.data.products || []);
    } catch (err) {
    ...
    <div style={{ backgroundColor: '#f0f0f0', minHeight: '100vh' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Posters</h1>
          <p className="text-gray-600">Large format prints for maximum visual impact</p>
        </div>
          ...
          {posters.map((product, index) => (
            ...
            >
              ...
              <div
                ...
                style={{ height: '200px', backgroundColor: product.bg || '#c9b8d4' }}
              >
                <div className="flex items-center justify-center w-full h-full px-8">
                  <div className="bg-white rounded-xl shadow-md w-full max-w-[140px] h-[180px] p-4 flex flex-col gap-3">
