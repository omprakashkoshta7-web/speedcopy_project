...
const FlyersListPage: React.FC = () => {
  ...
  const fetchFlyers = async () => {
    try {
      setLoading(true);
      const response = await productService.getBusinessProducts({ category: 'flyers', limit: 12 });
      setFlyers(response.data.products || []);
    } catch (err) {
    ...
    <div style={{ backgroundColor: '#f0f0f0', minHeight: '100vh' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Flyers & Leaflets</h1>
          <p className="text-gray-600">High-impact marketing materials for your campaigns</p>
        </div>

        {/* Products Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
          {flyers.map((product, index) => (
            <div
              ...
              >
                <div className="flex items-center justify-center w-full h-full px-8">
                  <div className="bg-white rounded-xl shadow-md w-full max-w-[180px] p-4 flex flex-col gap-2">
                    ...
                    <div className="h-2 rounded-full bg-gray-100 w-4/6" />
